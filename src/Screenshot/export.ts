// ZSnip 截图插件 - 数据导出（使用 ZTools 官方 API）
...
function renderToCanvas(sel: Selection): HTMLCanvasElement {
    ...
    c.width = sel.width * dpr
    c.height = sel.height * dpr
    ...
    if (screenshotBase64) {
        ...
        ctx.drawImage(img, sel.x, sel.y, sel.width, sel.height, 0, 0, sel.width, sel.height)
    }
    // 绘制所有标注（偏移到选区坐标系）
...
function offsetShape(shape: AnyShape, sel: Selection): AnyShape {
    switch (shape.type) {
        case 'rect':
            return { ...shape, x: shape.x - sel.x, y: shape.y - sel.y } as RectShape
        case 'circle':
            return { ...shape, cx: shape.cx - sel.x, cy: shape.cy - sel.y } as CircleShape
        case 'arrow':
            return { ...shape, x1: shape.x1 - sel.x, y1: shape.y1 - sel.y, x2: shape.x2 - sel.x, y2: shape.y2 - sel.y } as ArrowShape
        case 'pen':
            return { ...shape, points: shape.points.map(p => ({ x: p.x - sel.x, y: p.y - sel.y })) } as PenShape
        case 'text':
            return { ...shape, x: shape.x - sel.x, y: shape.y - sel.y } as TextShape
        case 'mosaic':
            return { ...shape, x: shape.x - sel.x, y: shape.y - sel.y } as MosaicShape
    }
}

/**
 * 复制截图到剪贴板 - 使用 ZTools 官方 API
 */
export async function copyToClipboard(): Promise<boolean> {
    ...
    try {
        ...
        // 使用官方 copyImage API（通过 preload services）
...
export async function saveToFile(): Promise<string | null> {
    ...
    try {
        const c = renderToCanvas(selection)
        const base64 = c.toDataURL('image/png')
        // 使用官方 getPath + fs.writeFileSync（通过 preload services）
